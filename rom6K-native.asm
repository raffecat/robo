...
Disp     = $D0    ; Dispatch Pointer low (BASIC)
DispH    = $D1    ; Dispatch Pointer high (BASIC)
...
; parse the line
  ...
  JSR skip_spc      ; [6] Y=ofs -> Y, A=next-char
...
; try commands
  ...
  JSR match_kw      ; [6] Y=ofs A=index X=page -> Y, CF=found, A=hi-byte (uses A,X,B,Src)
...
match_kws:       ; Y=ofs A=az-index X=page -> Y, CF=found, A=hi-byte (uses A,X,B,Src)
...
@skip_lp:        ; no match
  ...
  ASL            ; [2] test bit 6 ($40 continue bit)
  BMI @next_kw   ; [2] -> bit 6 set, try next keyword [+1]
  CLC            ; [2] CC=not-found
  RTS            ; [6] return
@matched:        ; found match
  AND #$BF       ; [2] clear bit 6 ($40 marker)
  SEC            ; [2] CS=found
  RTS            ; [6] return
...
tok_var:            ; 
  ...
  BCS e_syn0        ; -> syntax error (expecting VAR)
@lp:
  JSR emit_byte     ; emit it
; check for alpha
  JSR skip_spc      ; A=next-char
  TAX               ; save A
  JSR is_alpha      ; A=char -> A=az-index, CC=alphabetic (preserves X,Y)
...
; check for digit
  TXA               ; restore A
  JSR is_digit      ; A=char -> A=az-index, CC=alphabetic (preserves X,Y)
  BCC @lp           ; -> continue VAR
  RTS

; @@ typ_chk
; ensure E and X hold the same types
...
; match a keyword   ; U_KW,>stmt_page,<bKWLet,U_DUP,U_OUT
  ...
  JSR match_kws     ; [6] Y=ofs A=index X=page -> Y, CS=found, A=hi-byte (uses A,X,B,Src)
  BCC syn_to_let    ; [2] -> implied LET statement
  ORA #$C0          ; [2] set top bits ($Cx opcodes)
  JSR emit_byte     ; [6] emit opcode
; dispatch syntax handler
  AND #$3F          ; [2] clear top bits ($C0)
  TAX               ; [2] as index
...
tok_fnret:
  ...
  LDA #OP_FNRET
  JSR emit_byte
  JMP tok_exp       ; expression, then return
...
; check for ELSE (can appear after any statement)
  ...
  JSR match_kw      ; Y=ofs A=index X=page -> Y, CS=found, A=hi-byte (uses A,X,B,Src)
  BCC e_syn0        ; -> no match, syntax error
  BPL syn_else_op   ; -> found ELSE
@eol:
...
syn_for:
  ...
  JSR match_kw      ; [6] Y=ofs A=index X=page -> Y, CS=found, A=hi-byte (uses A,X,B,Src)
...
; check for THEN
  ...
  BCS syn_goln      ; [2] -> ALWAYS: implied goto
...
syn_print:
  ...
  ORA #$C0          ; set top bits ($Cx opcodes)
  JSR emit_byte     ; emit opcode (uses X; preserves A,Y -> NE,PL)
...
; @@ tok_exp
; parse an expression
tok_exp:            ; (uses A,X,B,C) -> X=type (0=num 1=str)
  LDX #0            ; type 0=num
  STX C             ; result type (D=count,E=type used by callers)
  STX D             ; nesting depth = 0
tok_exp2:
  LDA #<kwi_Prefx   ; A=prefix table
  JSR match_kw      ; Y=ofs A=index X=page -> Y, CS=found, A=hi-byte (uses A,X,B,Src)
  BCC @noop         ; -> no operator
  CMP #$80          ; unary plus?
  BEQ tok_led       ; -> no opcode
  JSR emit_byte     ; emit opcode (uses X; preserves A,Y -> PL)
  CMP #OP_PAREN     ; parenthesis?
  BEQ @paren        ; -> handle parentheses
  CMP #OP_STRL      ; string literal?
  BNE tok_led       ; -> expect operator
; string literal
  JSR emit_str      ; Y=ofs -> Y, NE
  INC C             ; type 1=str
  BNE tok_led       ; -> ALWAYS: expect operator
@paren:
  INC D             ; increase nesting depth               (XXX conflicts with D=count)
  BNE tok_exp2      ; -> expect nested expression
@noop:
  LDA LineBuf,Y     ; A=next-char
  JSR is_alpha      ; A=char -> A=az-index, CC=alphabetic (preserves X,Y)
  BCC @varfn        ; -> VAR or FUN
  TXA
  JSR is_digit      ; A=char -> A=digit, CC=numeric (preserves X,Y)
  BCC @num          ; -> number literal
  JMP e_syn1        ; -> syntax error
@varfn:             ; A=az-index
  LDX #>expr_page   ; X=page
  JSR match_kws     ; Y=ofs A=az-index X=page -> Y, CS=found, A=hi-byte (uses A,X,B,Src)
  BCC @var          ; -> must be a VAR
  JSR emit_byte     ; emit opcode ($80)
; dispatch function handler
  AND #$3F          ; clear top bits ($80)
  TAX               ; as index
  LDA #>fun_page    ; X = high byte (page)
  PHA               ; push high byte
  LDA disp_fun,X    ; A = function handler offset (NOTE minus 1)
  PHA               ; push low byte minus 1
  JMP skip_spc      ; -> skip_spc, then RTS to the handler -> NE (from skip_spc)
@var:
  JSR syn_var       ; -> VAR[$][(..)] -> X/E=type

@num:               ; encode and emit number


; expression left-denom (expect operator)
tok_led:
...
tab_prfn  DB "SPC", OP_SPC+$40  ; bit 6 set, continue (+$40)
          ...
          DB "AT",  OP_AT+$40   ; bit 6 set, continue (+$40)
tab_else  DB "ELSE", OP_ELSE    ; bit 6 clear, (~$40 final)

tab_prefx DB "-", OP_UNEG+$40   ; unary minus (+$40 continue)
          DB "+", $80+$40       ; unary plus (+$40 continue)
          DB "(", OP_PAREN+$40  ; parenthesis (+$40 continue)
          DB "NOT", OP_NOT+$40  ; unary NOT (+$40 continue)
          DB 34, OP_STRL        ; `"` string literal (~$40 final)

tab_infx  DB "+", OP_ADD+$40    ; infix add (+$40 continue)
          DB "-", OP_SUB+$40    ; infix subtract (+$40 continue)
          DB "*", OP_MUL+$40    ; infix multiply (+$40 continue)
          DB "/", OP_DIV+$40    ; infix divide (+$40 continue)
          DB "^", OP_POW+$40    ; infix power (+$40 continue)
          DB ")", $FF           ; close bracket (+$40 continue)
          DB "<>", OP_NE+$40    ; infix not equal (+$40 continue)
          DB "<=", OP_LE+$40    ; infix less-equal (+$40 continue)
          DB "<", OP_LT+$40    ; infix less-than (+$40 continue)
          DB ">=", OP_GE+$40    ; infix greater-equal (+$40 continue)
          DB ">", OP_GT+$40    ; infix greater-than (+$40 continue)
          DB "AND", OP_AND+$40  ; infix AND (+$40 continue)
          DB "OR", OP_OR+$40    ; infix OR (+$40 continue)
          DB "EOR", OP_EOR+$40  ; infix EOR (+$40 continue)
          DB "DIV", OP_OR+$40   ; infix DIV (+$40 continue)
          DB "MOD", OP_OR       ; infix MOD (~$40 final)
...
; @@ set_prog
; set up to run BASIC statements
set_prog:
  STA CODEH         ;
  LDY #0            ; code offset
...
OP_I0    = $10       ; 1-byte
OP_INT2  = $1A       ; 2-byte (opc|int8)
OP_INT3  = $1B       ; 3-byte (opc|int16)
OP_INT4  = $1C       ; 4-byte (opc|int24)

OP_FLT2  = $0C       ; 2-byte (opc|mant8)         [".5"; "0.255"]
OP_FLT3  = $0D       ; 3-byte (opc|exp8|mant8)    ["1.2"; "10.2"; "2.55"]
OP_FLT4  = $0E       ; 4-byte (opc|exp8|mant16)   ["99.9", "1.9876", "6.5535"]
OP_FLT5  = $0F       ; 5-byte (opc|exp8|mant24)   ["99.99", "987654.3", "9.876543", "1.6777215"]
...
eval_n:
  ...
  BCC @numpf     ; [2] -> number ($1x) or prefix ($2x) [12]
; --- look up variable [11]
  STY B          ; [3] save Y = CodeOfs
  JSR find_var   ; [6] find VAR matching Y=ofs -> Ptr=var, Y=slot-ofs, A=tag, B=CodeOfs; report not found
  AND #1         ; [2] num/str flag
  BNE e_typ1     ; [2] -> not numeric [+1]
  LDA (Ptr),Y    ; [5] Num0
...
@bfunc:          ; [8]
  AND #$3F       ; [2] keep low 6 bits (clear $80)
  TAX            ; [2]
  LDA fntabl,X   ; [4] func ptr low (minus 1)
  STA Disp       ; [3] set address low byte
  JMP (Disp)     ; [5] -> jump to opcode [24]
...
; @@ gfx_move
; move to (X,Y)      XXX can't take stack params
gfx_move:
  PLA
  STA GfxPtX
  PLA
...
; @@ gfx_plot
; draw a point at (X,Y)
; all params are u8 on the stack: [00XXXXXX][00YYYYYY]<--    XXX can't take stack params
...
; @@ gfx_line
; draw a Bresenham line (X0,Y0) to (X1,Y1)        XXX can't take stack params
