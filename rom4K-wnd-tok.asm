...
wrchr:           ; A=char; (uses A,X,Src,Dst,F) preserves Y [25]
  ...
  DEC WinRem     ; [5] at right edge of window?               (4b) WinRem
  BEQ @nl        ; [2] if so -> newline [+1]   
;  LDA TXTP       ; [5] get new TXTP                           (6b) NoRem
;  AND #31        ; [2] test low 5 bits
;  BEQ @nl        ; [2] if so -> newline [+1]
  RTS            ; [6]
@nl:
;  INC WinRem     ; [5] let newline do it, so it can detect TXTPH increment (fix!)   (window)  (-2b)
...
; clear the bottom line
  ...
  TAY              ; window width                        (window)
  STY WinRem       ; reset WinRem                        (window)
  DEY              ; minus 1                             (window)
@clear:
...
@left:           ; move back one cell
  LDA WinRem     ; [3] remaining space on this line               (window)
  CMP WinW       ; [3] window width                               (window)
  BEQ @left_sol  ; [2] equal -> at start of this line [+1]
  INC WinRem     ; [5] give back one character                    (window)
...
@left_sol:       ; start of buffer, or some line within a buffer?
  ...
  LDA TXTP
  SEC            ;                                                (window)
  SBC WinAdv     ; to end of prior line (CC if wrapped)           (window)
...
; @@ print_xa
; print a string at XA=address with Y=length
print_xa:          ; XA=ptr, Y=len -> X=0, Y=len (uses A,X,Y,B,Ptr,Src,Dst)
  STA Ptr          ; source address low
  STX PtrH         ; source address high
  BNE print        ; -> print it
...
vid_mode:        ; set screen mode, A=mode (uses A,X,Y,B)
  ...
  STA WinAdv     ; advance to reach next line (32 - WinW)                  (window)
  LDA #32        ;                                                         (window)
...
txt_addr_xy:     ; A=row X=col -> AY=addr (uses A,X,Y)
  JSR txt_addr   ; AX -> AY
  STA TXTPH      ; set TXTPH
  STY TXTP       ; set TXTP
  RTS            ; 
...
txt_addr:          ; A=row X=col -> AY=addr (uses A,X,Y,F)
  STX F            ; [000XXXXX] -> F
  ROR              ; [0000YYYY]Y
  ROR              ; [Y0000YYY]Y
  ROR              ; [YY0000YY]Y
...
; A = Row    [000YYYYY]
txt_addr2:         ; A=row X=col -> AY=addr (uses A,X,Y,F) [18]
  LDX #0
  STX F
  ASL     ;  [00YYYYY0]
  ASL     ;  [0YYYYY00]
...
old_boot:
  LDY #<msg_boot
  JSR printmsgln  ; (uses A,B,C,D,X,Y,Src,Dst)          5
  ; detect memory installed
  ; XX scan memmap
;  STA TopPtrH      ; also reset TOP
;  LDX #0           ; low
...
; memory detect
  ...
  STA EndPage    ; set end of memory (will be dynamic)
  LDA #>Base4K   ; page
  STA BasePage   ; set BASIC start address (will be dynamic)   8
; free memory
  STX AccE         ; AccE=0 to display free memory      
  STX Acc2         ; Acc2=0 to display free memory
  SEC
...
tok_stmts:
  ...
  LDX #>stmt_page    ; stmt table page
  JSR match_kws      ; Y=ofs XA=table -> CF=found, Y=ofs, A=high-byte (uses A,X,Y,B,C,D,Src)
  BCC syn_let        ; -> no match, must be VAR (use LET handler)
  JSR tok_emit       ; output token with top-bit set (uses X=0; preserves A,Y; sets NE)
  AND #$63           ; low 6 bits, statement index
  CMP #stmt_count    ; ASSERT: is it < stmt_count
  BCS tok_syn        ; ASSERT: -> out of bounds
